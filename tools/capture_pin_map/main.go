...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage: capture_pin_map <address>")
		os.Exit(1)
	}
	...
	filename := fmt.Sprintf("pin_map_%s.png", sanitizeFilename(address))
	...
	// 3. Generate a map with a pin at the address location
	// 4. Save the image to outputPath
